...
const updatesUrl = 'https://www.naeu.playblackdesert.com/en-US/News/Notice?boardType=5';

const getName = (element: any) => _.get(element, 'children[3].children[1].children')
  .filter((x: any) => x.attribs && x.attribs.class === 'desc');
...
const getPatchNotesNew = async (args: any, history: number) => {
  try {
    ...
    const mappedLinks = newsLinks.map((x) => `${x.name[0].children[0].data}\n${x.link}`);
